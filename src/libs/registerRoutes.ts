...
const registerRoutes = async (app: App) => {
  for (const key of Object.keys(modules)) {
    ...
    const controllersPath = path.resolve(`src/modules/${moduleName}/controller`)
    const controllers = await fs.promises.readdir(controllersPath)

    for (const controller of controllers) {
      const routersModule = await import(path.resolve(`src/modules/${moduleName}/controller/${controller}`))
